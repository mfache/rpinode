...
/*
 * ADAPTATION BOITIER (cf. README § 16.6) :
 *
 * Copie de drivers/usb/serial/ti_usb_3410_5052.c (noyau Linux mainline,
 * inchangee depuis, licence d'origine ci-dessous) avec DEUX modifications :
 *
 *   1. Dans ti_startup(), le Moxa UPort 1150/1150I (comme les 1130/1130I
 *      deja geres par le pilote d'origine) est force en mode RS-485 2 fils
 *      (`td_rs485_only = true`) au lieu du defaut RS-232 codé en dur pour ce
 *      modele. Le pilote mainline gere deja le firmware Moxa
 *      (moxa/moxa-1150.fw) et la commande vendor TI_SET_CONFIG qui pilote ce
 *      mode electrique (bUartMode) ; il ne l'active simplement jamais pour
 *      le 1150/1151, qui supporte plusieurs modes (232/422/485) alors que
 *      les 1130/1131 sont RS-485/422 uniquement. Aucun pilote hors-arbre
 *      Moxa (mxu11x0) n'est necessaire : celui-ci ne compile de toute facon
 *      plus sur les noyaux recents (API usb_serial_driver trop ancienne) et
 *      depend d'un firmware/protocole que le pilote mainline gere deja.
 *   2. kzalloc_obj()/kmalloc_obj() : ce fichier a ete recupere sur une
 *      branche mainline legerement plus recente que ce noyau (6.18.34+rpt) ;
 *      ces macros y prennent un argument GFP explicite ici (ex.
 *      `kzalloc_obj(struct ti_device, GFP_KERNEL)`), contre un seul argument
 *      sur des noyaux plus recents. Purement une question de compatibilite
 *      de macro, aucun changement de comportement.
 *
 * Compile en module hors-arbre (meme .ko, memes alias USB, meme vermagic)
 * via build_moxa_rs485_driver.sh, qui REMPLACE le module mainline installe
 * (sauvegarde en .ko.xz.orig) : se recharge automatiquement au prochain
 * branchement, sans action manuelle (setserial etc.) cote utilisateur.
 *
 * -----------------------------------------------------------------------
 * En-tete/licence d'origine (Linux mainline) :
 *
